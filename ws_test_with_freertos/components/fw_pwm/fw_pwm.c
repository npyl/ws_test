...
|-----------------------------------------------------------------------------|
|                                                                             |
|   Module Name  : fw_pwm.c Module File.                                      |
|                                                                             |
...
/*
 * WARNING:
 *   Must have correct pinmux from Halcogen prior to setting these.
 *   All PWM channels are defined for use and controlled by the .enabled field.
 */
static const S_ETPMW_CONFIG pwm_ch_config_defs[eETPWM_MAX] =
{
    /* eETPWM_0 */
    {
        .id = eETPWM_0,
        .label = "ETPWM_1",
        .rise_delay_source = ePWM_A,
        .rise_delay = 10,
        .fall_delay_source = ePWM_A,
        .fall_delay = 10,
        .ch = {
            {   /* Channel A */
                .id = ePWM_A,
...
void etpwm_init( void )
{
    int i;
    etpwmBASE_t * p_etpwm;
    const S_ETPMW_CONFIG * const p_cfg = pwm_ch_config_defs;

    for ( i = 0; i < eETPWM_MAX; i++ )
    {
        if ( p_cfg[ i ].enabled == TRUE )
        {
            /* Get the ETPWM base register using the PWM macro */
            p_etpwm = ETPWM( p_cfg[ i ].id );

            /* Sets high speed time-base clock prescale bits */
            p_etpwm->TBCTL = ( uint16 ) 0U << 7U;

            /* Sets time-base clock prescale bits */
            p_etpwm->TBCTL |= ( uint16 ) ( ( uint16 ) 0U << 10U );

            /* Sets time period for ETPWM block both PWMA and PWMB */
            /* Using channel A period as the common period */
            p_etpwm->TBPRD = p_cfg[ i ].ch[ ePWM_A ].period;

            /* Setup the duty cycle for PWMA */
            /* Convert duty percentage to counter value */
            uint16_t duty_a = ( uint16_t ) ( ( p_cfg[ i ].ch[ ePWM_A ].period * p_cfg[ i ].ch[ ePWM_A ].duty ) / 100 );
            p_etpwm->CMPA = duty_a;

            /* Setup the duty cycle for PWMB */
            /* Convert duty percentage to counter value */
            uint16_t duty_b = ( uint16_t ) ( ( p_cfg[ i ].ch[ ePWM_B ].period * p_cfg[ i ].ch[ ePWM_B ].duty ) / 100 );
            p_etpwm->CMPB = duty_b;

            /* Configure channel A polarity and action qualification */
            uint16_t aq_zero_a = p_cfg[ i ].ch[ ePWM_A ].polarity ? ActionQual_Set : ActionQual_Clear;
            uint16_t aq_cmp_a = p_cfg[ i ].ch[ ePWM_A ].polarity ? ActionQual_Clear : ActionQual_Set;

            /* Force EPWMxA output based on configured polarity when counter reaches zero and compare A value */
            p_etpwm->AQCTLA = ( ( uint16 ) ( ( uint16 ) aq_zero_a << 0U ) |
                                ( uint16 ) ( ( uint16 ) aq_cmp_a << 4U ) );

            /* Configure channel B polarity and action qualification */
            uint16_t aq_zero_b = p_cfg[ i ].ch[ ePWM_B ].polarity ? ActionQual_Set : ActionQual_Clear;
            uint16_t aq_cmp_b = p_cfg[ i ].ch[ ePWM_B ].polarity ? ActionQual_Clear : ActionQual_Set;

            /* Force EPWMxB output based on configured polarity when counter reaches zero and compare B value */
            p_etpwm->AQCTLB = ( ( uint16 ) ( ( uint16 ) aq_zero_b << 0U ) |
                                ( uint16 ) ( ( uint16 ) aq_cmp_b << 8U ) );

            /* Mode setting for Dead Band Module */
            uint16_t db_rise_src = p_cfg[ i ].rise_delay_source == ePWM_A ? 0U : 1U;
            uint16_t db_fall_src = p_cfg[ i ].fall_delay_source == ePWM_A ? 0U : 1U;
            uint16_t invert_a = p_cfg[ i ].ch[ ePWM_A ].invert_polarity ? 1U : 0U;
            uint16_t invert_b = p_cfg[ i ].ch[ ePWM_B ].invert_polarity ? 1U : 0U;
            uint16_t rise_delay_en = p_cfg[ i ].ch[ ePWM_A ].delay_en || p_cfg[ i ].ch[ ePWM_B ].delay_en ? 1U : 0U;
            uint16_t fall_delay_en = p_cfg[ i ].ch[ ePWM_A ].delay_en || p_cfg[ i ].ch[ ePWM_B ].delay_en ? 1U : 0U;

            p_etpwm->DBCTL = ( ( uint16 ) ( ( uint16 ) db_fall_src << 5U )      /* Source for Falling edge delay */
                            |  ( uint16 ) ( ( uint16 ) db_rise_src << 4U )      /* Source for Rising edge delay */
                            |  ( uint16 ) ( ( uint16 ) invert_b << 3U )         /* Enable/Disable EPWMxB invert */
                            |  ( uint16 ) ( ( uint16 ) invert_a << 2U )         /* Enable/Disable EPWMxA invert */
                            |  ( uint16 ) ( ( uint16 ) rise_delay_en << 1U )    /* Enable/Disable Rising Edge Delay */
                            |  ( uint16 ) ( ( uint16 ) fall_delay_en << 0U ) ); /* Enable/Disable Falling Edge Delay */

            /* Set the rising edge delay */
            p_etpwm->DBRED = p_cfg[ i ].rise_delay;

            /* Set the falling edge delay */
            p_etpwm->DBFED = p_cfg[ i ].fall_delay;

            /* Enable the chopper module for ETPWMx - set to disabled as default */
            p_etpwm->PCCTL = ( ( uint16 ) ( ( uint16 ) 0U << 0U )    /* Enable/Disable chopper module */
            ...
            /* Set trip source enable - disabled by default */
            p_etpwm->TZSEL = 0x0000U;

            /* Set interrupt enable - disabled by default */
            p_etpwm->TZEINT = 0x0000U;

            /* Sets up the event for interrupt - NO_EVENT by default */
            p_etpwm->ETSEL = (uint16)NO_EVENT;

            if ( (p_etpwm->ETSEL & 0x0007U) != 0U )
            {
                p_etpwm->ETSEL |= 0x0008U;
            }

            /* Setup the frequency of the interrupt generation */
            p_etpwm->ETPS = 1U;

            /* Sets up the ADC SOC interrupt - using DCAEVT1 and DCBEVT1 by default */
            p_etpwm->ETSEL |= ( ( uint16 ) ( 0x0000U )
                             | ( uint16 ) ( 0x0000U )
                             | ( uint16 ) ( ( uint16 ) DCAEVT1 << 8U )
                             | ( uint16 ) ( ( uint16 ) DCBEVT1 << 12U ) );

            /* Sets up the ADC SOC period */
            p_etpwm->ETPS |= ( ( uint16 ) ( ( uint16 ) 1U << 8U )
...
{
    /* Return the address of the ETPWM configuration data */
    return pwm_ch_config_defs;
}
