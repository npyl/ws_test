...
|-----------------------------------------------------------------------------|
|                                                                             |
|   Module Name  : fw_can.c Module File.                                      |
|                                                                             |
...
static const S_CAN_CONFIG can_config_defs[ eCAN_MAX ] =
{
...
static uint8 can_rx_buffer[ eCAN_MAX ][ sizeof( S_CAN_INFO ) ];
static uint8 can_rx_buffer_idx[ eCAN_MAX ];
...
void can_init( void )
{
    ...
    xCANQueueHandle = xQueueCreate( CAN_QUEUE_LENGTH, CAN_QUEUE_ITEM_SIZE );
    const S_CAN_CONFIG * const p_cfg = canGetConfig();

    for ( i = 0; i < eCAN_MAX; i++ )
    {
        p_can = CAN( i );
        if ( p_cfg[ i ].enabled == TRUE )
        {
            p_can->CTL = ( uint32 ) 0x00000000U
                         | ( uint32 ) 0x00000000U
                         | ( uint32 ) ( ( uint32 ) 0x00000005U  << 10U )
                         | ( uint32 ) 0x00020043U;

            /** - Clear all pending error flags and reset current status */
            p_can->ES |= 0xFFFFFFFFU;

            /** - Assign interrupt level for messages */
            p_can->INTMUXx[ 0U ] = ( uint32 ) 0x00000000U;

            p_can->INTMUXx[ 1U ] = ( uint32 ) 0x00000000U;

            /** - Setup auto bus on timer period */
            p_can->ABOTR = ( uint32 ) 0U;

            while ( ( p_can->IF1STAT & 0x80U ) == 0x80U )
            {
            }
...
{
    /* Return the address of the CAN configuration data */
    return can_config_defs;
}

void canMessageNotification( canBASE_t *node, uint32 messageBox )
{
    E_CAN_ID id = CAN_IDX( node );

    canGetData( node, canMESSAGE_BOX2, ( uint8 * ) &can_rx_buffer[ id ] );
}
